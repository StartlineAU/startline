...
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'organisers_customerId_key') THEN
    ALTER TABLE "organisers" RENAME CONSTRAINT "organisers_customerId_key" TO "organisers_userId_key";
  END IF;
END $$;
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'organisers_customerId_key') THEN

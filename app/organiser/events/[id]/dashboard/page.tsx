      ...
      <div className="min-h-screen bg-dark">
        ...
        <main className="pt-24">
          <div className="max-w-[1100px] mx-auto px-6 py-16 text-center">
            <div className="w-6 h-6 border-2 border-white/[0.08] border-t-white/60 rounded-full animate-spin mx-auto" />
          </div>
        </main>
      </div>
    );
      ...
      <div className="min-h-screen bg-dark">
        ...
        <main className="pt-24">
          <div className="max-w-[1100px] mx-auto px-6 py-16 text-center">
            ...
            <div className="text-white/50 text-sm mb-6">{error || "This event's dashboard is unavailable."}</div>
    ...
    <div className="min-h-screen bg-dark">
      ...
      <main className="pt-24">
        <div className="max-w-[1100px] mx-auto px-4 sm:px-6 py-6 sm:py-8 pb-24 lg:pb-12 page-in">
          ...
          {/* Breadcrumb */}
          <div className="flex items-center gap-2 font-headline text-[11px] uppercase tracking-widest text-white/40 mb-6">
            <Link href="/organiser/listings" className="hover:text-white/70 transition-colors">Listings</Link>
          ...
          <div className="flex flex-col lg:flex-row lg:items-start gap-6 mb-8">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex flex-wrap items-center gap-2 mb-2">
                <Badge className="bg-lime-500/15 text-lime-400 border-0">
                  Live
                </Badge>
                <span className="font-headline text-[11px] uppercase tracking-widest text-white/40">
          ...
          <div className="flex flex-col sm:flex-row gap-4 mb-6">
            ...
            <Card className="flex-1">
              <CardContent className="p-5 flex items-center gap-4">
                ...
                <div>
                  <div className="font-headline text-[11px] font-bold uppercase tracking-widest text-white/40 mb-0.5">Registered</div>
                  <div className="font-headline text-2xl font-black italic tracking-tighter text-light">
                    {event.registrationCount.toLocaleString()}
                    {event.cap && <span className="text-white/40 font-normal text-lg"> / {event.cap.toLocaleString()}</span>}
                  </div>
                  {capacityPct !== null && (
                    <div className="mt-2 h-1 w-32 bg-dark-light rounded-full overflow-hidden">
                      <div
            ...
            <Card className="flex-1">
              <CardContent className="p-5 flex items-center gap-4">
                ...
                <div>
                  <div className="font-headline text-[11px] font-bold uppercase tracking-widest text-white/40 mb-0.5">Estimated payout</div>
                  <div className="font-headline text-2xl font-black italic tracking-tighter text-light">
                    {fmt(payout.estimatedPayout)}
                  </div>
                  <div className="text-[11px] text-white/40 mt-0.5">after platform fees</div>
                </div>
              </CardContent>
            </Card>
          </div>
                  ...
                  {event.waves.map((w, i) => (
                    <div key={i} className="flex items-center justify-between py-3 gap-4">
                      ...
                      <div className="flex items-center gap-4 sm:gap-6 shrink-0">
                        <div className="font-headline text-[14px] font-black italic text-light">A${w.price}</div>
                        <div className="text-right">
                          <div className="font-headline text-[10px] uppercase tracking-widest text-white/40">Cap</div>
                          <div className="font-headline text-[13px] text-white/60">{w.qty ? w.qty.toLocaleString() : "—"}</div>
                        </div>
                      </div>
                    </div>
                  ))}
          ...
          <Card className="mb-8">
            <CardContent className="p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <Badge className="bg-dark-light text-white/60 border-0">
              ...
              {event.registrationCount > 0 ? (
                <div className="text-center py-8 text-white/40">
                  ...
                  <div className="font-headline text-sm font-bold uppercase tracking-widest text-white/40 mb-1">
                ...
                <div className="text-center py-8">
                  <Users className="w-8 h-8 mx-auto mb-3 text-white/30" />
                  <div className="font-headline text-sm font-bold uppercase tracking-widest text-white/40 mb-1">No registrations yet</div>
                  <div className="text-[13px] text-white/40">Athletes who register will appear here.</div>
                </div>
              )}
            </CardContent>
          </Card>
            ...
            <CardContent className="p-6">
              <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-6">
                <div>
                  ...
                  <p className="text-[13px] text-white/40 mt-0.5 max-w-sm">
              ...
              {/* Compose panel */}
              {showCompose && (
                <div className="mb-6 p-5 bg-dark-darker border border-white/[0.08] rounded-xl">
                  <div className="font-headline text-[13px] font-bold uppercase tracking-widest text-white/70 mb-4 flex items-center gap-2">
                    <Megaphone className="w-4 h-4" /> New announcement
                  </div>
                  {postError && (
                    <div className="mb-3 px-3 py-2 bg-red-500/15 border border-red-500/30 rounded-lg text-[12px] text-red-400">{postError}</div>
                  )}
                  <div className="space-y-3">
                    <div>
                      <label className="font-headline text-[11px] font-bold uppercase tracking-widest text-white/60 block mb-1.5">Title</label>
                      <input
                        type="text"
                        value={annTitle}
                        onChange={e => setAnnTitle(e.target.value)}
                        placeholder="e.g. Schedule update — Saturday heat times"
                        className="w-full bg-dark border border-white/[0.08] rounded-lg px-3 py-2.5 text-[14px] text-light placeholder:text-white/40 focus:border-white/20 focus:outline-none transition-colors"
                      />
                    </div>
                    <div>
                      <label className="font-headline text-[11px] font-bold uppercase tracking-widest text-white/60 block mb-1.5">Message</label>
                      <textarea
                        rows={4}
                        value={annBody}
                        onChange={e => setAnnBody(e.target.value)}
                        placeholder="Write your announcement here…"
                        className="w-full bg-dark border border-white/[0.08] rounded-lg px-3 py-2.5 text-[14px] text-light placeholder:text-white/40 focus:border-white/20 focus:outline-none resize-none transition-colors"
                      />
                    </div>
              ...
              {announcements.length === 0 ? (
                <div className="text-center py-10">
                  ...
                  <div className="font-headline text-sm font-bold uppercase tracking-widest text-white/40 mb-1">No announcements yet</div>
                  <div className="text-[13px] text-white/40">Post an update to notify registered athletes.</div>
                </div>
              ) : (
                <div className="space-y-4">
                  {announcements.map(ann => (
                    <div key={ann.id} className="flex items-start gap-4 p-4 bg-dark border border-white/[0.05] rounded-xl">
                      <div className="w-8 h-8 rounded-lg bg-lime-500/15 flex items-center justify-center shrink-0 mt-0.5">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between gap-3">
                          ...
                          <div className="font-headline text-[11px] uppercase tracking-widest text-white/40 shrink-0">{timeAgo(ann.createdAt)}</div>
                        </div>
                        <p className="text-[13px] text-white/60 mt-1 leading-relaxed whitespace-pre-wrap">{ann.body}</p>
                      </div>
                      <button
                        onClick={() => setDelAnn(ann)}
                        className="shrink-0 text-white/30 hover:text-red-500 transition-colors mt-0.5"

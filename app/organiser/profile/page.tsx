                      ...
                      return (
                        <Link key={e.id} href={`/organiser/events/${e.id}/dashboard`}
                          ...
                          <div className="relative aspect-[16/10] bg-dark-light overflow-hidden">
                            {ef.coverImageUrl(e)
                              ? <img src={ef.coverImageUrl(e)!} alt={ef.title(e)} className="w-full h-full object-cover brightness-[.62] saturate-110 group-hover:scale-105 transition-transform duration-300" />
                              : <div className="absolute inset-0 bg-dark-light flex items-center justify-center">
                                  <span className="font-headline font-black italic text-muted-dark text-4xl tracking-tighter">{ef.discipline(e).slice(0, 4).toUpperCase()}</span>
                                </div>}

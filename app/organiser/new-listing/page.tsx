...
/* â”€â”€ Shared field primitive â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
  ...
  return (
    <div className="mb-6">
      <div className="flex items-baseline justify-between mb-2">
        ...
        {hint && <span className="font-headline text-[10px] uppercase tracking-widest text-muted">{hint}</span>}
      </div>
      {children}
    </div>
  );
...
function DatePickerPopover({
  ...
  const displayValue = isRange
    ? value
      ? rangeEnd && rangeEnd !== value ? `${fmtDateShort(value)} â€” ${fmtDateShort(rangeEnd)}`
        : rangeEnd && rangeEnd === value ? fmtDateShort(value) + " (1 day)"
        : fmtDateShort(value) + " â†’ pick end date"
      : ""
    ...
    <div ref={ref} className="relative">
      <button type="button"
        ...
        className={`w-full bg-dark-light border rounded-md px-4 py-3 text-[15px] text-left flex items-center justify-between transition-colors
          ${open ? "border-primary" : "border-dark-lighter hover:border-dark-lighter"}
          ${displayValue ? "text-light" : "text-muted"}`}>
        <span className="flex items-center gap-2.5 min-w-0">
          <Calendar className="w-4 h-4 text-muted shrink-0" />
          <span className="truncate">{displayValue || placeholder}</span>
        </span>
        <ChevronDown className={`w-4 h-4 text-muted shrink-0 ml-2 transition-transform duration-200 ${open ? "rotate-180" : ""}`} />
      </button>

      {open && (
        <div className="absolute top-full left-0 mt-2 z-50 bg-dark-light border border-dark-lighter rounded-xl shadow-xl p-4 w-full sm:w-72 modal-in">
          {isRange && (
            <div className="mb-3 flex items-center justify-between">
              <span className={`font-headline text-[10px] uppercase tracking-widest font-bold px-2 py-1 rounded-md ${picking === "start" ? "bg-primary/10 text-primary" : "text-muted"}`}>
                {picking === "start" ? "â–¸ Tap start date" : "â–¸ Tap end date"}
              </span>
              {value && !rangeEnd && (
          ...
          <div className="grid grid-cols-7 mb-1">
            {["Mo","Tu","We","Th","Fr","Sa","Su"].map(d => (
              <div key={d} className="font-headline text-[9px] uppercase tracking-widest text-muted text-center py-1">{d}</div>
            ))}
          </div>
                ...
                <div key={i} className={`flex items-center justify-center h-9 ${inRange ? "bg-primary/10" : ""} ${start && (rangeEnd || picking === "end") ? "bg-gradient-to-r from-transparent to-primary/10" : ""} ${end ? "bg-gradient-to-l from-transparent to-primary/10" : ""}`}>
                  <button type="button" disabled={past} onClick={() => selectDay(d)}
                    className={`w-9 h-9 rounded-full text-[13px] font-headline font-bold transition-colors
                      ${start || sel ? "bg-primary text-dark" : end ? "bg-primary/80 text-dark" : inRange ? "text-primary hover:bg-primary/10" : past ? "text-muted-dark opacity-50 cursor-not-allowed" : today ? "text-primary border border-primary/40 hover:bg-primary/10" : "text-muted-light hover:bg-white/5 hover:text-primary"}`}>
                    {d}
                  </button>
                </div>
              );
      ...
      <div className="relative flex items-center">
        ...
        {value && (
          <button type="button" onClick={(e) => { e.stopPropagation(); onChange(""); setOpen(false); }}
            className="absolute right-9 p-1.5 text-muted hover:text-muted-light transition-colors" title="Clear time">
            <X className="w-3.5 h-3.5" />
          </button>
        )}
      </div>
      {open && (
        <div className="absolute top-full left-0 mt-2 z-50 bg-dark-light border border-dark-lighter rounded-xl shadow-xl overflow-hidden w-56 modal-in">
          <div className="p-3 border-b border-dark-lighter">
            <div className="font-headline text-[9px] uppercase tracking-widest text-muted mb-1.5">Custom time</div>
          ...
          <div className="px-4 py-2 font-headline text-[9px] uppercase tracking-widest text-muted text-center">â€” or pick a slot â€”</div>
...
function RichTextEditor({ value, onChange }: { value: string; onChange: (html: string) => void }) {
  ...
  return (
    <div className="border border-dark-lighter rounded-md overflow-hidden focus-within:border-primary transition-colors">
      {/* Toolbar */}
      <div className="flex items-center gap-1 px-3 py-2 border-b border-dark-lighter bg-dark-light flex-wrap">
        ...
        <div className="w-px h-5 bg-dark-lighter mx-1" />
        <button type="button" title="Bullet list" onClick={() => exec("insertUnorderedList")} className={`${toolbarBtn} text-[11px]`}>â€¢ List</button>
      </div>
      {/* Editable area */}
      <div
        ref={editorRef}
        ...
        data-placeholder="Tell athletes what makes this event unmissable â€” course details, atmosphere, divisions, what to bringâ€¦"
        className="min-h-[220px] px-4 py-3 text-[14px] text-light focus:outline-none prose prose-sm max-w-none
          ...
          [&_h4]:font-headline [&_h4]:font-bold [&_h4]:text-[14px] [&_h4]:text-muted-light [&_h4]:mt-2 [&_h4]:mb-1
...
const DISCIPLINES: { v: Discipline; l: string; d: string }[] = [
  ...
  { v: "running",   l: "Running",   d: "5K Â· 10K Â· Half Â· Marathon" },
  { v: "hybrid",    l: "Hybrid",    d: "Multi-discipline / OCR"     },
  { v: "cycling",   l: "Cycling",   d: "Road Â· Criterium Â· Gravel"  },
  { v: "swimming",  l: "Swimming",  d: "Pool Â· Open water events"   },
            ...
            return (
              <button key={f.v} type="button" onClick={() => update({ format: f.v })}
                className={`flex sm:flex-col items-center sm:items-start gap-3 sm:gap-1 text-left p-3 sm:p-4 rounded-md border transition-all
                  ${on ? "border-primary bg-primary/10" : "border-dark-lighter hover:border-dark-lighter"}`}>
          ...
          {DISCIPLINES.map(d => {
            ...
            return (
              <button key={d.v} type="button" onClick={() => update({ discipline: d.v, categories: [] })}
                className={`text-left p-4 rounded-md border transition-all ${on ? "border-primary bg-primary/10" : "border-dark-lighter hover:border-dark-lighter"}`}>
          ...
          <div className="flex flex-wrap gap-2">
            {(DISCIPLINE_CATS[form.discipline as Discipline] ?? []).map(c => (
              <button key={c} type="button" onClick={() => toggle(c)}
                className={`font-headline text-[11px] font-bold uppercase tracking-widest px-3 py-2 rounded-md border transition-colors
                  ${form.categories.includes(c) ? "border-primary bg-primary/10 text-primary" : "border-dark-lighter text-muted hover:border-dark-lighter hover:text-muted-light"}`}>
            ...
            {showCustomCat ? (
              <div className="flex items-center gap-2">
                ...
                <button type="button" onClick={() => { setShowCustomCat(false); setCustomCatInput(""); }} className="text-muted hover:text-muted-light transition-colors"><X className="w-4 h-4" /></button>
              </div>
            ) : (
              <button type="button" onClick={() => setShowCustomCat(true)}
                ...
                <Plus className="w-3 h-3 inline mr-1" /> Customâ€¦
              </button>
            )}
          </div>
        </Field>
      ...
      <Field label="Participant cap" required hint="Max registrations">
        <div className="flex flex-wrap gap-2 mb-3">
            ...
            return (
              <button key={c} type="button" onClick={() => { update({ cap: c }); setCapMode("preset"); }}
                className={`font-headline text-[12px] font-bold uppercase tracking-widest px-4 py-2.5 rounded-md border transition-colors
                  ${active ? "border-primary bg-primary/10 text-primary" : "border-dark-lighter text-muted hover:border-dark-lighter hover:text-muted-light"}`}>
          ...
          <button type="button" onClick={() => setCapMode("custom")}
            className={`font-headline text-[12px] font-bold uppercase tracking-widest px-4 py-2.5 rounded-md border transition-colors
              ${capMode === "custom" ? "border-primary bg-primary/10 text-primary" : "border-dark-lighter text-muted hover:border-dark-lighter hover:text-muted-light"}`}>
            Custom
          </button>
        </div>
        {capMode === "custom" && (
      ...
      <Field label="Minimum age" required>
        <div className="flex flex-wrap gap-2 mb-3">
          <button type="button" onClick={() => { update({ minAge: "0" }); setAgeMode("open"); }}
            className={`font-headline text-[12px] font-bold uppercase tracking-widest px-4 py-2.5 rounded-md border transition-colors
              ${ageMode === "open" ? "border-primary bg-primary/10 text-primary" : "border-dark-lighter text-muted hover:border-dark-lighter hover:text-muted-light"}`}>
            Open to all
          </button>
          {AGE_PRESETS.map(a => (
            <button key={a} type="button" onClick={() => { update({ minAge: a }); setAgeMode("preset"); }}
              className={`font-headline text-[13px] font-bold uppercase tracking-widest px-5 py-2.5 rounded-md border transition-colors
                ${ageMode === "preset" && form.minAge === a ? "border-primary bg-primary/10 text-primary" : "border-dark-lighter text-muted hover:border-dark-lighter hover:text-muted-light"}`}>
              {a}+
            </button>
          ))}
          <button type="button" onClick={() => { update({ minAge: "" }); setAgeMode("custom"); }}
            className={`font-headline text-[12px] font-bold uppercase tracking-widest px-4 py-2.5 rounded-md border transition-colors
              ${ageMode === "custom" ? "border-primary bg-primary/10 text-primary" : "border-dark-lighter text-muted hover:border-dark-lighter hover:text-muted-light"}`}>
            Custom
          </button>
        </div>
        {ageMode === "custom" && (
          <div className="flex items-center gap-3">
            <div className="flex items-center border border-dark-lighter rounded-md overflow-hidden">
              <button type="button" onClick={() => update({ minAge: String(Math.max(0, (parseInt(form.minAge) || 0) - 1)) })}
                className="w-9 h-11 flex items-center justify-center text-muted hover:text-light hover:bg-white/5 transition-colors font-headline text-lg select-none">âˆ’</button>
              ...
              <button type="button" onClick={() => update({ minAge: String((parseInt(form.minAge) || 0) + 1) })}
                className="w-9 h-11 flex items-center justify-center text-muted hover:text-light hover:bg-white/5 transition-colors font-headline text-lg select-none">+</button>
            </div>
            <span className="font-headline text-[13px] text-muted">years old minimum</span>
          </div>
        )}
        {ageMode === "open" && (
          <p className="font-headline text-[11px] uppercase tracking-widest text-muted">No age restriction â€” open to all ages.</p>
        )}
      </Field>
    </div>
  );
...
/* â”€â”€ State select â€” custom dropdown matching city autocomplete style â”€â”€ */
    ...
    <div ref={ref} className="relative">
      <button
        ...
        className={`w-full bg-dark-light border rounded-md px-4 py-3 text-[15px] text-left flex items-center justify-between transition-colors
          ...
          ${value ? "text-light" : "text-muted"}`}
      >
        <span>{selected ? `${selected[1]}` : "Select stateâ€¦"}</span>
        <ChevronDown className={`w-4 h-4 text-muted shrink-0 transition-transform duration-200 ${open ? "rotate-180" : ""}`} />
      </button>

      {open && (
        <div className="absolute top-full left-0 mt-2 z-50 w-full bg-dark-light border border-dark-lighter rounded-xl shadow-xl overflow-y-auto max-h-72 modal-in">
          {rows.map(([v, abbr, full]) => (
            ...
            >
              ...
              <span className="font-headline text-[12px] text-muted">{full}</span>
            </button>
          ))}
        </div>
      )}
    ...
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5">
        <Field label="Start time" required>
          ...
          <p className="font-headline text-[10px] uppercase tracking-widest text-muted mt-1.5">
    ...
    <div>
            ...
            return (
              <button key={value} type="button" onClick={() => update({ registrationType: value })}
                className={`flex flex-col items-start gap-1 rounded-xl border-2 px-5 py-4 text-left transition-colors
                  ${active ? "border-primary bg-primary/10" : "border-dark-lighter bg-dark-light hover:border-dark-lighter"}`}>
        ...
        {form.registrationType === "startline" && (
          <div className="mt-3 space-y-2">
            <div className="flex items-center justify-between mt-1">
              <div className="font-headline text-[10px] uppercase tracking-widest text-muted">Fee structure</div>
              <div className="font-headline text-[10px] uppercase tracking-widest text-muted">Startline fee: 3.95% + A$1.45 per ticket</div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {([
            ...
            <div key={i} className="bg-dark-light border border-dark-lighter rounded-lg p-4 space-y-3">
              <div className="flex items-center gap-3">
                ...
                <button onClick={() => removeWave(i)}
                  className="w-9 h-9 rounded text-muted hover:text-primary hover:bg-white/5 flex items-center justify-center transition-colors shrink-0">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    <div className="font-headline text-[10px] uppercase tracking-widest text-muted">Price (A$)</div>
                    <label className="flex items-center gap-1.5 cursor-pointer select-none">
                      <span className="font-headline text-[10px] uppercase tracking-widest text-muted">Free</span>
                      <div onClick={() => updateWave(i, { price: w.price === "0" ? "" : "0" })}
                        ...
                        <div className={`absolute top-0.5 w-3 h-3 rounded-full bg-dark-light shadow transition-transform duration-200 ${w.price === "0" ? "translate-x-4" : "translate-x-0.5"}`} />
                      </div>
                    </label>
                  </div>
                  {w.price === "0" ? (
                    <div className="w-full bg-primary/10 border border-primary/20 rounded-md px-4 py-3 font-headline text-[13px] font-bold uppercase tracking-widest text-primary">Free</div>
                  ) : (
                  ...
                  {form.registrationType === "startline" && (() => {
                    ...
                    return (
                      <div className="mt-2 rounded-md bg-dark-light/5 px-3 py-2.5 space-y-1">
                        {([
                ...
                <div>
                  <div className="font-headline text-[10px] uppercase tracking-widest text-muted mb-1.5">Category closes</div>
              ...
              {/* Per-wave start time */}
              <div>
                <div className="font-headline text-[10px] uppercase tracking-widest text-muted mb-1.5 flex items-center gap-1.5">
                  <Clock className="w-3 h-3" /> Wave start time <span className="text-muted-dark">â€” optional</span>
                </div>
      ...
      <div className="border border-dark-lighter rounded-xl overflow-hidden mb-6">
        <button type="button" onClick={() => update({ prizeMoney: !form.prizeMoney })}
          className="w-full flex items-center justify-between px-5 py-4 bg-dark-light hover:bg-white/5 transition-colors">
          <div>
            ...
            <div className="font-headline text-[10px] uppercase tracking-widest text-muted mt-0.5 text-left">This event offers a cash prize pool</div>
          </div>
          <div className={`relative w-11 h-6 rounded-full transition-colors shrink-0 ${form.prizeMoney ? "bg-primary/100" : "bg-dark-lighter"}`}>
            <span className={`absolute top-0.5 left-0.5 w-5 h-5 rounded-full bg-dark-light shadow transition-transform ${form.prizeMoney ? "translate-x-5" : "translate-x-0"}`} />
          </div>
        </button>
        {form.prizeMoney && (
          <div className="px-5 pb-5 pt-3 bg-dark-light border-t border-dark-lighter">
            <div className="relative">
              <span className="absolute left-3 top-1/2 -translate-y-1/2 font-headline text-[13px] text-muted">A$</span>
      ...
      <Field label="Refund & transfer policy" required>
        ...
        <textarea rows={2} value={refundCustom} onChange={e => handleRefundCustom(e.target.value)}
          placeholder="Additional details, deferral windows, exceptionsâ€¦" className={textareaCls} />
      </Field>
    </div>
  );
}
      ...
      <Field label="Event images" required hint="First image is used as cover">
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          {allImageSrcs.map((src, idx) => (
            <div key={idx} className="relative aspect-video rounded-md overflow-hidden border border-dark-lighter group">
              ...
              <button type="button" onClick={() => removeImage(idx)}
                className="absolute top-1.5 right-1.5 w-6 h-6 rounded-full bg-gray-900/70 text-muted-dark hover:text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                <X className="w-3 h-3" />
              </button>
            </div>
          ))}
          ...
          <label className="aspect-video rounded-md border-2 border-dashed border-dark-lighter hover:border-primary/40 bg-dark-light flex flex-col items-center justify-center cursor-pointer transition-colors group">
            ...
            <span className="font-headline text-[10px] uppercase tracking-widest text-muted group-hover:text-primary transition-colors">
        ...
        <p className="font-headline text-[10px] uppercase tracking-widest text-muted mt-2">JPG Â· PNG Â· WEBP Â· Recommended 1920Ã—1080</p>
      </Field>
...
function ReviewStep({ form, setStep, confirmed, onConfirm }: {
  ...
  const rows: { k: string; v: string; step: number }[] = [
    { k: "Title",          v: form.title || "â€”",                                                                   step: 0 },
    { k: "Discipline",     v: form.discipline ? form.discipline.toUpperCase() : "â€”",                               step: 0 },
    { k: "Format",         v: form.format || "â€”",                                                                  step: 0 },
    { k: "Intensity",      v: form.level ? INTENSITY_LABELS[form.level] : "â€”",                                    step: 0 },
    { k: "Cap / Min age",  v: `${form.cap ? parseInt(form.cap).toLocaleString() : "â€”"} Â· ${form.minAge === "0" ? "Open to all" : form.minAge ? `${form.minAge}+` : "â€”"}`, step: 0 },
    { k: "Date",           v: form.date
        ? form.endDate && form.endDate !== form.date
          ? `${new Date(form.date + "T00:00:00").toLocaleDateString("en-AU", { day:"numeric", month:"short", year:"numeric" })} â€” ${new Date(form.endDate + "T00:00:00").toLocaleDateString("en-AU", { day:"numeric", month:"short", year:"numeric" })}`
          : new Date(form.date + "T00:00:00").toLocaleDateString("en-AU", { day:"numeric", month:"short", year:"numeric" })
        : "â€”",                                                                                                      step: 1 },
    { k: "Start / End",    v: form.startTime ? `${fmt24to12(form.startTime)}${form.endTime ? ` â†’ ${fmt24to12(form.endTime)}` : ""}` : "â€”", step: 1 },
    { k: "Venue",          v: `${form.venue || "â€”"}, ${form.city || "â€”"}, ${form.state ? form.state.toUpperCase() : "â€”"}`, step: 1 },
    { k: "Tickets",        v: `${form.waves.length} categor${form.waves.length !== 1 ? "ies" : "y"}, from ${form.waves[0]?.price === "0" ? "Free" : form.waves[0]?.price ? `A$${form.waves[0].price}` : "â€”"}`, step: 2 },
    { k: "Registration",   v: form.registrationType === "startline" ? "Startline" : form.registrationUrl || "â€”",  step: 2 },
    { k: "Refund policy",  v: form.refundPolicy || "â€”",                                                            step: 2 },
    ...
    { k: "Description",    v: form.description ? `${form.description.replace(/<[^>]+>/g, "").trim().slice(0, 60)}â€¦` : "â€”", step: 3 },
  ];

  return (
    <div>
      <div className="bg-dark-light border border-dark-lighter rounded-lg overflow-hidden mb-6">
        {rows.map((r, i) => (
          <div key={r.k} className={`flex items-center gap-4 px-5 py-4 ${i === rows.length - 1 ? "" : "border-b border-dark-lighter/50"}`}>
...
function LivePreview({ form }: { form: FormState }) {
  ...
  const sDay = sp[2] || "â€”";
  const sMon = sp[1] ? MONTHS_SHORT[parseInt(sp[1]) - 1] : "â€”";
  const eDay = ep[2];
  const eMon = ep[1] ? MONTHS_SHORT[parseInt(ep[1]) - 1] : null;
  const price = form.waves.find(w => w.price === "0" || !!w.price)?.price;

  const dateLabel = form.date
    ? form.endDate && form.endDate !== form.date ? `${sDay} ${sMon} â€” ${eDay} ${eMon}` : `${sDay} ${sMon}${sp[0] ? ` ${sp[0]}` : ""}`
    : "Date TBC";
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <span className="font-headline text-[11px] font-bold uppercase tracking-widest text-primary">Live preview</span>
      </div>
        ...
        <div className="relative h-52 placeholder-stripes scan-grid flex items-center justify-center overflow-hidden">
          ...
          <div className="absolute bottom-4 left-4 right-4">
            ...
            <div className="font-headline text-lg font-black italic tracking-tighter text-light leading-tight line-clamp-2">
              {form.title || <span className="text-muted-dark/60">Event titleâ€¦</span>}
            </div>
          </div>
        </div>
        <div className="p-5">
          <div className="space-y-3">
            ...
            <div className="flex items-start gap-3">
              ...
              <span className="font-headline text-[11px] uppercase tracking-widest text-muted leading-snug">
                {dateLabel}{form.startTime ? ` Â· ${fmt24to12(form.startTime)}` : ""}
              </span>
            </div>
            {form.format && (
              <div className="flex items-start gap-3">
...
function EventFullPreview({ form, onClose }: { form: FormState; onClose: () => void }) {
  const discipline  = DISC_LABEL[form.discipline] || "â€”";
  const stateLabel  = form.state.toUpperCase();
  const formatLabel = FORMAT_LABELS_PREVIEW[form.format] || "â€”";
  const intensity   = form.level ? INTENSITY_LABELS[form.level] : "â€”";
  ...
  const dateLabel = (() => {
    ...
    if (form.endDate && form.endDate !== form.date) {
      ...
      return `${d.toLocaleDateString("en-AU", { day: "numeric", month: "long" })} â€“ ${e.toLocaleDateString("en-AU", opts)}`;
    }
      ...
      <div className="relative flex flex-col w-full h-full overflow-hidden modal-in">
        <div className="relative z-10 flex items-center justify-between px-5 py-3 bg-dark-darker/95 backdrop-blur border-b border-dark-lighter shrink-0">
          <div className="flex items-center gap-3">
            ...
            <span className="font-headline text-[10px] uppercase tracking-widest text-muted-dark hidden sm:block">â€” This is how your listing will appear to athletes</span>
          </div>
                ...
                <div className="bg-dark rounded-xl p-5 border-l-4 border-primary flex flex-col gap-4">
                  <div>
                    ...
                    {form.discipline
                      ...
                      : <span className="text-muted-dark text-[12px]">â€”</span>}
                  </div>
                  <div>
...
export default function NewListingPage() {
  ...
  const STEP_HEADINGS = [
    { h: <>Let&apos;s start with<br /><span className="text-primary">the basics.</span></>, sub: "Name, format, discipline and intensity â€” the essentials every athlete will see first." },
    ...
    <div className="min-h-screen bg-dark">
      ...
      <div className="pt-14">
        <div className="anim-fade-slide">
                    ...
                    <div key={s.k} className="flex items-center flex-1 min-w-0">
                      <button onClick={() => goTo(i)}
                        ...
                        <div className={`relative w-8 h-8 rounded-md border flex items-center justify-center font-headline font-black italic text-[13px] flex-shrink-0
                          ${cur ? "bg-primary text-dark border-primary" : hasErr ? "bg-orange-50 text-orange-500 border-orange-300" : done ? "bg-dark-light/5 text-primary border-primary/30" : "bg-dark-light border-dark-lighter text-muted"}`}>
                        ...
                        <div className="hidden xl:block min-w-0">
                          ...
                          <div className={`font-headline text-[10px] uppercase tracking-widest truncate ${hasErr ? "text-orange-400" : "text-muted"}`}>
          ...
          <div className="max-w-[1280px] mx-auto grid lg:grid-cols-[1fr_360px]">
                    ...
                    <ul className="space-y-2.5">
                      {submitErrors.map(i => (
                        <li key={i} className="flex items-start justify-between gap-4 py-2.5 px-3 rounded-lg bg-orange-500/5 border border-orange-500/10">
                          <div>
                            <p className="font-headline text-[12px] font-bold uppercase tracking-widest text-orange-600">{STEPS[i].n} â€” {STEPS[i].label}</p>
              ...
              <div className="mt-6 flex items-center justify-between pt-5 border-t border-dark-lighter">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button onClick={next} disabled={saving || (step === STEPS.length - 1 && !confirmed)}
                    ...
                    {saving
                      ? <><span className="w-2 h-2 bg-dark rounded-full animate-pulse-dot" /> Savingâ€¦</>
                      : step === STEPS.length - 1
            ...
            {/* Live preview sidebar */}
            <aside className="hidden lg:block border-l border-dark-lighter bg-dark-light p-6 sticky top-[152px] h-[calc(100vh-152px)] overflow-y-auto">
      ...
      {showCancelModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 overlay-in">
          ...
          <div className="relative bg-dark-light border border-dark-lighter rounded-2xl shadow-2xl w-full max-w-sm p-7 modal-in">
            ...
            <div className="flex flex-col gap-3">
              <button onClick={async () => { setShowCancelModal(false); await submitToApi(true, form.title.trim() || "Untitled draft"); router.push("/organiser/dashboard"); }}
                ...
                {saving ? "Savingâ€¦" : <><Check className="w-4 h-4" /> Save draft &amp; leave</>}
              </button>
              <button onClick={() => router.push("/organiser/dashboard")}
                className="w-full font-headline text-[13px] font-bold uppercase tracking-widest px-6 py-3.5 rounded-md border border-dark-lighter text-muted hover:text-light hover:border-dark-lighter transition-colors">
                Discard &amp; leave
              </button>
              <button onClick={() => setShowCancelModal(false)}
                className="font-headline text-[12px] uppercase tracking-widest text-muted hover:text-muted transition-colors text-center py-1">
                Keep editing
              </button>
            </div>
          </div>

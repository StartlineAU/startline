  ...
  return (
    <div className="mb-6">
      <div className="flex items-baseline justify-between mb-2">
        <label className="font-headline text-[11px] font-bold uppercase tracking-widest text-white/70">
          {label}{required && <span className="text-lime-600 font-black text-[15px] leading-none ml-1">*</span>}
        </label>
        {hint && <span className="font-headline text-[10px] uppercase tracking-widest text-white/40">{hint}</span>}
      </div>
      {children}
    </div>
  );
...
const inputCls    = "w-full bg-dark border border-white/[0.08] rounded-md px-4 py-3 text-[15px] text-light placeholder:text-white/40 focus:border-lime-500 focus:outline-none transition-colors";
const textareaCls = "w-full bg-dark border border-white/[0.08] rounded-md px-4 py-3 text-[14px] text-light placeholder:text-white/40 focus:border-lime-500 focus:outline-none resize-none transition-colors";
    ...
    <div ref={ref} className="relative">
      <button
        ...
        className={`w-full bg-dark border rounded-md px-4 py-3 text-[15px] text-left flex items-center justify-between transition-colors
          ${open ? "border-lime-500" : "border-white/[0.08] hover:border-white/[0.08]"}
          ${displayValue ? "text-light" : "text-white/40"}`}
      >
      ...
      {open && (
        <div className="absolute top-full left-0 mt-2 z-50 bg-dark border border-white/[0.08] rounded-xl shadow-xl p-4 w-full sm:w-72 modal-in">
          {/* Range mode hint */}
          {isRange && (
            <div className="mb-3 flex items-center justify-between">
              <span className={`font-headline text-[10px] uppercase tracking-widest font-bold px-2 py-1 rounded-md
                ${picking === "start" ? "bg-lime-500/15 text-lime-400" : "text-white/40"}`}>
                {picking === "start" ? "▸ Tap start date" : "▸ Tap end date"}
              </span>
              {value && !rangeEnd && (
                <button type="button" onClick={() => { onChange(""); if (onChangeEnd) onChangeEnd(""); setPicking("start"); }}
                  className="font-headline text-[10px] uppercase tracking-widest text-white/50 hover:text-lime-600 transition-colors">
                  Reset
                </button>
              )}
            </div>
          ...
          <div className="flex items-center justify-between mb-4">
            <button type="button" onClick={prevMonth}
              className="w-9 h-9 rounded-md hover:bg-dark-light flex items-center justify-center text-white/50 hover:text-lime-600 transition-colors">
          ...
          <div className="grid grid-cols-7 mb-1">
            {["Mo","Tu","We","Th","Fr","Sa","Su"].map(d => (
              <div key={d} className="font-headline text-[9px] uppercase tracking-widest text-white/40 text-center py-1">{d}</div>
            ))}
          </div>
                ...
                >
                  <button
                    ...
                    className={`w-9 h-9 rounded-full text-[13px] font-headline font-bold transition-colors
                      ...
                      : inRange        ? "text-lime-400 hover:bg-lime-500/15"
                      : past           ? "text-white/30 opacity-50 cursor-not-allowed"
                      : today          ? "text-lime-600 border border-lime-400 hover:bg-lime-500/15"
                      :                  "text-white/70 hover:bg-dark-light hover:text-lime-600"}`}
                  >
                    {d}
                  </button>
                </div>
          ...
          <div className="mt-3 pt-3 border-t border-white/[0.08] flex items-center justify-between">
            <button type="button" onClick={() => { onChange(""); if (onChangeEnd) onChangeEnd(""); setOpen(false); setPicking("start"); }}
              className="font-headline text-[11px] uppercase tracking-widest text-white/50 hover:text-lime-600 transition-colors">
              Clear
            </button>
    ...
    <div ref={ref} className="relative">
      <div className="relative flex items-center">
        <button
          ...
          className={`w-full bg-dark border rounded-md px-4 py-3 text-[15px] text-left flex items-center justify-between transition-colors
            ${open ? "border-lime-500" : "border-white/[0.08] hover:border-white/[0.08]"}
            ${value ? "text-light" : "text-white/40"}`}
        >
        ...
        {value && (
          <button
            ...
            className="absolute right-9 p-1.5 text-white/40 hover:text-white/70 transition-colors"
            title="Clear time"
          >
      ...
      {open && (
        <div className="absolute top-full left-0 mt-2 z-50 bg-dark border border-white/[0.08] rounded-xl shadow-xl overflow-hidden w-56 modal-in">
          {/* Custom time input */}
          <div className="p-3 border-b border-white/[0.08]">
            <div className="font-headline text-[9px] uppercase tracking-widest text-white/40 mb-1.5">Custom time</div>
            <div className="flex gap-2">
              <input
                ...
                className={`flex-1 min-w-0 bg-dark-darker border rounded px-3 py-2 text-[13px] text-light placeholder:text-white/40 focus:outline-none transition-colors
                  ${customError ? "border-red-400 focus:border-red-500" : "border-white/[0.08] focus:border-lime-500"}`}
              />
              <button
                type="button"
          ...
          {/* Divider */}
          <div className="px-4 py-2 font-headline text-[9px] uppercase tracking-widest text-white/40 text-center">
            — or pick a slot —
          </div>
              ...
              <button
                ...
                className={`w-full px-4 py-3 text-left font-headline text-[14px] font-bold transition-colors
                  ${s === value
                    ...
                    : "text-white/50 hover:bg-dark-darker hover:text-light"}`}
              >
                {fmt24to12(s)}
              </button>
            ))}
    ...
    <div>
      <Field label="Event date(s)" required hint="Tap start then end for multi-day">
        ...
        {form.endDate && form.endDate !== form.date && (
          <button
            ...
            className="mt-1.5 font-headline text-[10px] uppercase tracking-widest text-white/40 hover:text-lime-600 transition-colors flex items-center gap-1"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5">
        <Field label="City" required>
          <div className={`${inputCls} ${form.city ? "text-light" : "text-white/40"}`}>
            {form.city || "—"}
          </div>
        </Field>
        <Field label="State" required>
          <div className={`${inputCls} ${form.state ? "text-light" : "text-white/40"}`}>
            ...
            return (
              <button key={f.v} type="button" onClick={() => update({ format: f.v })}
                className={`flex sm:flex-col items-center sm:items-start gap-3 sm:gap-1 text-left p-3 sm:p-4 rounded-md border transition-all min-h-[44px]
                  ${on ? "border-lime-500 bg-lime-500/15" : "border-white/[0.08] hover:border-white/[0.08]"}`}>
                <div className={`font-headline text-[14px] font-black italic tracking-tighter ${on ? "text-lime-400" : "text-light"}`}>{f.l}</div>
                <div className="font-headline text-[10px] uppercase tracking-widest text-white/50">{f.d}</div>
              </button>
            );
          })}
        </div>
          ...
          {DISCIPLINES.map((d) => {
            ...
            return (
              <button key={d.v} type="button" onClick={() => update({ discipline: d.v, categories: [] })}
                className={`text-left p-4 rounded-md border transition-all ${on ? "border-lime-500 bg-lime-500/15" : "border-white/[0.08] hover:border-white/[0.08]"}`}>
                <div className={`font-headline text-[15px] font-black italic tracking-tighter ${on ? "text-lime-400" : "text-light"}`}>{d.l}</div>
                <div className="font-headline text-[10px] uppercase tracking-widest text-white/50 mt-1">{d.d}</div>
              </button>
            );
          })}
        </div>
          ...
          <div className="flex flex-wrap gap-2">
            {(DISCIPLINE_CATS[form.discipline as Discipline] ?? []).map((c) => (
              <button key={c} type="button" onClick={() => toggle(c)}
                className={`font-headline text-[11px] font-bold uppercase tracking-widest px-3 py-2 rounded-md border transition-colors ${form.categories.includes(c) ? "border-lime-500 bg-lime-500/15 text-lime-400" : "border-white/[0.08] text-white/50 hover:border-white/[0.08] hover:text-white/70"}`}>
            ...
            {form.categories.filter(c => !(DISCIPLINE_CATS[form.discipline as Discipline] ?? []).includes(c)).map(c => (
              <button key={c} type="button" onClick={() => toggle(c)}
                className="font-headline text-[11px] font-bold uppercase tracking-widest px-3 py-2 rounded-md border border-lime-500 bg-lime-500/15 text-lime-400">
            ...
            {showCustomCat ? (
              ...
              <button type="button" onClick={() => setShowCustomCat(true)}
                className="font-headline text-[11px] font-bold uppercase tracking-widest px-3 py-2 rounded-md border border-white/[0.08] text-white/50 hover:border-lime-500 hover:bg-lime-500/15 hover:text-lime-400 transition-colors">
        ...
        <div className="flex flex-wrap gap-2">
          {LEVELS.map((l) => (
            <button key={l.v} type="button" onClick={() => update({ level: l.v })}
              className={`font-headline text-[12px] font-bold uppercase tracking-widest px-4 py-2.5 rounded-md border transition-colors ${form.level === l.v ? "border-lime-500 bg-lime-500/15 text-lime-400" : "border-white/[0.08] text-white/50 hover:border-white/[0.08] hover:text-white/70"}`}>
              {l.l}
            </button>
          ))}
        </div>
      ...
      <Field label="Participant cap" hint="Max registrations">
        <div className="flex flex-wrap gap-2 mb-3">
          {CAP_PRESETS.map(c => {
            ...
            return (
              <button key={c} type="button"
                ...
                className={`font-headline text-[12px] font-bold uppercase tracking-widest px-4 py-2.5 rounded-md border transition-colors ${active ? "border-lime-500 bg-lime-500/15 text-lime-400" : "border-white/[0.08] text-white/50 hover:border-white/[0.08] hover:text-white/70"}`}>
          ...
          <button type="button"
            ...
            className={`font-headline text-[12px] font-bold uppercase tracking-widest px-4 py-2.5 rounded-md border transition-colors ${capMode === "unlimited" ? "border-lime-500 bg-lime-500/15 text-lime-400" : "border-white/[0.08] text-white/50 hover:border-white/[0.08] hover:text-white/70"}`}>
            Unlimited
          </button>
          <button type="button"
            onClick={() => setCapMode("custom")}
            className={`font-headline text-[12px] font-bold uppercase tracking-widest px-4 py-2.5 rounded-md border transition-colors ${capMode === "custom" ? "border-lime-500 bg-lime-500/15 text-lime-400" : "border-white/[0.08] text-white/50 hover:border-white/[0.08] hover:text-white/70"}`}>
            Custom
          </button>
        </div>
        {capMode === "custom" && (
        ...
        {capMode === "unlimited" && (
          <p className="font-headline text-[11px] uppercase tracking-widest text-white/40">No cap — open registrations until you close manually.</p>
        )}
      </Field>
      ...
      <Field label="Minimum age" required>
        <div className="flex flex-wrap gap-2 mb-3">
          <button type="button"
            ...
            className={`font-headline text-[12px] font-bold uppercase tracking-widest px-4 py-2.5 rounded-md border transition-colors ${ageMode === "open" ? "border-lime-500 bg-lime-500/15 text-lime-400" : "border-white/[0.08] text-white/50 hover:border-white/[0.08] hover:text-white/70"}`}>
            Open to all
          </button>
          {AGE_PRESETS.map(a => {
            ...
            return (
              <button key={a} type="button"
                ...
                className={`font-headline text-[13px] font-bold uppercase tracking-widest px-5 py-2.5 rounded-md border transition-colors ${active ? "border-lime-500 bg-lime-500/15 text-lime-400" : "border-white/[0.08] text-white/50 hover:border-white/[0.08] hover:text-white/70"}`}>
                {a}+
              </button>
            );
          })}
          <button type="button"
            ...
            className={`font-headline text-[12px] font-bold uppercase tracking-widest px-4 py-2.5 rounded-md border transition-colors ${ageMode === "custom" ? "border-lime-500 bg-lime-500/15 text-lime-400" : "border-white/[0.08] text-white/50 hover:border-white/[0.08] hover:text-white/70"}`}>
            Custom
          </button>
        </div>
        {ageMode === "custom" && (
          <div className="flex items-center gap-3">
            <div className="flex items-center border border-white/[0.08] rounded-md overflow-hidden">
              <button type="button"
                ...
                className="w-9 h-11 flex items-center justify-center text-white/40 hover:text-light hover:bg-dark-darker transition-colors font-headline text-lg select-none">
                −
              </button>
              <input
                ...
                placeholder="0"
                className="w-16 bg-dark px-2 py-3 text-[15px] text-light text-center placeholder:text-white/40 focus:outline-none [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none"
              />
              <button type="button"
            ...
            <span className="font-headline text-[13px] text-white/50">years old minimum</span>
          </div>
        )}
        {ageMode === "open" && (
          <p className="font-headline text-[11px] uppercase tracking-widest text-white/40">No age restriction — open to all ages.</p>
        )}
      </Field>
    </div>
  );
    ...
    <div>
              ...
              <button
                ...
                className={`flex flex-col items-start gap-1 rounded-xl border-2 px-5 py-4 text-left transition-colors
                  ${active
                    ...
                    : "border-white/[0.08] bg-dark hover:border-white/[0.08]"}`}
              >
                <div className={`font-headline text-[13px] font-bold uppercase tracking-widest ${active ? "text-lime-400" : "text-light"}`}>
                  {title}
                </div>
                <div className="font-headline text-[10px] uppercase tracking-widest text-white/40">{sub}</div>
              </button>
            );
          })}
        </div>
        ...
        {form.registrationType === "startline" && (
          <div className="mt-3 space-y-2">
            <div className="flex items-center justify-between mt-1">
              <div className="font-headline text-[10px] uppercase tracking-widest text-white/40">Fee structure</div>
              <div className="font-headline text-[10px] uppercase tracking-widest text-white/40">Startline fee: 3.95% + A$1.45 per ticket</div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {([
                {
      ...
      <Field label="Ticket categories">
        <div className="space-y-3">
          {form.waves.map((w, i) => (
            <div key={i} className="bg-dark-darker border border-white/[0.08] rounded-lg p-4 space-y-3">
              {/* Row 1: number + category name */}
              <div className="flex items-center gap-3">
                <div className="w-7 h-7 rounded-md bg-dark-lighter flex items-center justify-center font-headline font-black italic text-lime-600 text-[13px] shrink-0">
                  {i + 1}
                </div>
                ...
                <button onClick={() => removeWave(i)}
                  className="w-9 h-9 rounded text-white/40 hover:text-lime-600 hover:bg-dark-light flex items-center justify-center transition-colors shrink-0">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    <div className="font-headline text-[10px] uppercase tracking-widest text-white/40">Price (A$)</div>
                    <label className="flex items-center gap-1.5 cursor-pointer select-none">
                      <span className="font-headline text-[10px] uppercase tracking-widest text-white/40">Free</span>
                      <div
                        onClick={() => updateWave(i, { price: w.price === "0" ? "" : "0" })}
                        className={`relative w-8 h-4 rounded-full transition-colors duration-200 cursor-pointer ${w.price === "0" ? "bg-lime-500" : "bg-dark-lighter"}`}>
                        <div className={`absolute top-0.5 w-3 h-3 rounded-full bg-dark shadow transition-transform duration-200 ${w.price === "0" ? "translate-x-4" : "translate-x-0.5"}`} />
                      </div>
                    </label>
                  </div>
                  {w.price === "0" ? (
                    <div className="w-full bg-lime-500/15 border border-lime-500/30 rounded-md px-4 py-3 font-headline text-[13px] font-bold uppercase tracking-widest text-lime-600">
                      Free
                    </div>
                  ) : (
                    <div className="relative">
                      <span className="absolute left-3 top-1/2 -translate-y-1/2 font-headline text-[13px] text-white/50">A$</span>
                  ...
                  {form.registrationType === "startline" && (() => {
                    ...
                    return (
                      <div className="mt-2 rounded-md bg-dark-light px-3 py-2.5 space-y-1">
                        {([
                          ...
                          <div key={r.label} className="flex items-baseline justify-between">
                            <span className="font-headline text-[13px] uppercase tracking-widest text-light">
                              {r.label}{r.sub && <span className="ml-1.5 normal-case text-[11px] text-white/50">({r.sub})</span>}
                            </span>
                            <span className={`font-headline text-[14px] font-bold ${r.muted ? "text-white/50" : "text-light"}`}>
                              {r.value}
                            </span>
                          </div>
                        ))}
                ...
                <div>
                  <div className="font-headline text-[10px] uppercase tracking-widest text-white/40 mb-1.5">Category closes</div>
                  <DatePickerPopover
                    value={w.closes}
          ...
          <button onClick={addWave}
            className="w-full border border-dashed border-white/[0.08] rounded-md py-3 font-headline text-[12px] uppercase tracking-widest text-white/50 hover:text-lime-600 hover:border-lime-400 flex items-center justify-center gap-2 transition-colors">
      ...
      <Field label="What's included">
        <div className="flex flex-wrap gap-2">
            ...
            return (
              <button key={item} type="button" onClick={() => toggleInclusion(item)}
                className={`font-headline text-[11px] font-bold uppercase tracking-widest px-3 py-2 rounded-md border transition-colors flex items-center gap-1.5
                  ${active ? "border-lime-500 bg-lime-500/15 text-lime-400" : "border-white/[0.08] text-white/50 hover:border-white/[0.08] hover:text-white/70"}`}>
          ...
          {activeInclusions.filter(i => !INCLUSION_PRESETS.includes(i)).map(item => (
            <button key={item} type="button" onClick={() => toggleInclusion(item)}
              className="font-headline text-[11px] font-bold uppercase tracking-widest px-3 py-2 rounded-md border border-lime-500 bg-lime-500/15 text-lime-400 flex items-center gap-1.5">
      ...
      {(() => {
        ...
        return (
          <div className="border border-white/[0.08] rounded-xl overflow-hidden mb-6">
            <button
              type="button"
              onClick={() => update({ activations: on ? "" : " " })}
              className="w-full flex items-center justify-between px-5 py-4 bg-dark hover:bg-dark-darker transition-colors"
            >
              <div>
                <div className="font-headline text-[13px] font-bold uppercase tracking-widest text-light text-left">Activations</div>
                <div className="font-headline text-[10px] uppercase tracking-widest text-white/40 mt-0.5 text-left">Brand activations, experiences &amp; sponsor zones</div>
              </div>
              <div className={`relative w-11 h-6 rounded-full transition-colors shrink-0 ${on ? "bg-lime-500" : "bg-dark-lighter"}`}>
                <span className={`absolute top-0.5 left-0.5 w-5 h-5 rounded-full bg-dark shadow transition-transform ${on ? "translate-x-5" : "translate-x-0"}`} />
              </div>
            </button>
            {on && (
              <div className="px-5 pb-5 pt-3 bg-dark-darker border-t border-white/[0.08]">
                <textarea
                  autoFocus
                  rows={3}
          ...
          <div className="border border-white/[0.08] rounded-xl overflow-hidden mb-6">
            ...
            >
              <div>
                ...
                <div className="font-headline text-[10px] uppercase tracking-widest text-white/40 mt-0.5 text-left">Add-ons athletes can purchase at checkout</div>
              </div>
              <div className={`relative w-11 h-6 rounded-full transition-colors shrink-0 ${on ? "bg-lime-500" : "bg-dark-lighter"}`}>
                <span className={`absolute top-0.5 left-0.5 w-5 h-5 rounded-full bg-dark shadow transition-transform ${on ? "translate-x-5" : "translate-x-0"}`} />
              </div>
          ...
          {(form.coverImage || form.coverImageUrl) ? (
            <div className="relative rounded-md border border-primary/40 overflow-hidden aspect-video">
              ...
              <button
                ...
                className="absolute top-3 right-3 w-8 h-8 rounded-full bg-dark-darker/70 text-white/30 hover:text-white flex items-center justify-center transition-colors"
              >
                <X className="w-4 h-4" />
              </button>
            ...
            <div className="relative rounded-md border-2 border-dashed border-white/[0.08] hover:border-lime-400 bg-dark-darker aspect-video flex flex-col items-center justify-center transition-colors">
              <div className="w-12 h-12 rounded-full bg-dark-lighter flex items-center justify-center mb-3">
              ...
              <div className="font-headline text-[11px] uppercase tracking-widest text-white/40 mt-1">or click to browse</div>
              <div className="mt-4 font-mono text-[10px] uppercase tracking-widest text-white/40">JPG · PNG · WEBP</div>
            </div>
          )}
...
function ReviewStep({ form, setStep, confirmed, onConfirm }: { form: FormState; setStep: (n: number) => void; confirmed: boolean; onConfirm: (v: boolean) => void }) {
  ...
  return (
    <div>
      <div className="bg-dark border border-white/[0.08] rounded-lg overflow-hidden mb-6">
        {rows.map((r, i) => (
          <div key={r.k} className={`flex items-center gap-4 px-5 py-4 ${i === rows.length - 1 ? "" : "border-b border-white/[0.05]"}`}>
            <div className="font-headline text-[11px] uppercase tracking-widest text-white/50 w-32 flex-shrink-0">{r.k}</div>
            <div className="flex-1 text-[14px] text-light truncate">{r.v}</div>
            <button onClick={() => setStep(r.step)}
              className="font-headline text-[11px] uppercase tracking-widest text-white/40 hover:text-lime-600 flex items-center gap-1 transition-colors">
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="font-headline text-[10px] uppercase tracking-widest text-white/40">
        </span>
      </div>
              ...
              <PreviewSection number="04" title="Registration & Tickets">
                ...
                <div className="mt-4 flex items-center gap-3">
                  {form.registrationType === "startline" ? (
                    ...
                    <div className="inline-flex items-center gap-2 bg-dark rounded-full px-4 py-2 border border-white/10">
    ...
    <div className="min-h-screen bg-dark">
      <div className="pt-24">
        ...
        <div className="anim-fade-slide">
          {/* Sticky header */}
          <div className="sticky top-16 z-30 bg-dark/95 backdrop-blur border-b border-white/[0.08]">
            <div className="max-w-[1280px] mx-auto px-6 lg:px-8 pt-3 pb-3">
              {/* Breadcrumb */}
              <div className="flex items-center gap-3 mb-2">
                <button onClick={() => setShowCancelModal(true)}
                  className="flex items-center gap-1.5 text-white/50 hover:text-lime-600 font-headline text-[11px] uppercase tracking-widest transition-colors">
                ...
                <div className="ml-auto flex items-center gap-3">
                  <button
                    ...
                    className="flex items-center gap-1.5 font-headline text-[11px] font-bold uppercase tracking-widest text-white/50 hover:text-lime-600 transition-colors"
                  >
                    ...
                    <div key={s.k} className="flex items-center flex-1 min-w-0">
                      ...
                      >
                        ...
                        <div className={`relative w-8 h-8 rounded-md border flex items-center justify-center font-headline font-black italic text-[13px] flex-shrink-0
                          ...
                          : done   ? "bg-dark-light text-lime-600 border-lime-400/50"
                          :          "bg-dark border-white/[0.08] text-white/40"}`}>
                        ...
                        <div className="hidden xl:block min-w-0">
                          <div className={`font-headline text-[11px] font-bold uppercase tracking-widest truncate ${cur ? "text-light" : hasErr ? "text-orange-500" : "text-white/50"}`}>
                            {s.label}
                          </div>
                          <div className={`font-headline text-[10px] uppercase tracking-widest truncate ${hasErr ? "text-orange-400" : "text-white/40"}`}>
                            {hasErr ? "Missing required fields" : s.sub}
                          </div>
                        </div>
                      </button>
                      {i < STEPS.length - 1 && (
                        <div className="flex-1 h-px mx-3 bg-dark-lighter" />
                      )}
                    </div>
                  );
                })}
          ...
          <div className="max-w-[1280px] mx-auto grid lg:grid-cols-[1fr_360px]">
            <div className="p-4 sm:p-6 lg:p-8 pb-32 lg:pb-10">
              <div key={step} className={direction === "forward" ? "step-forward" : "step-back"}>
                <div className="mb-6">
                  ...
                  <p className="text-white/50 mt-2 max-w-lg text-[14px]">
                ...
                {apiError && (
                  <div className="mt-4 px-4 py-3 rounded-md bg-red-500/15 border border-red-500/30 text-red-400 font-headline text-[13px]">
                    {apiError}
                  </div>
                )}
              ...
              <div className="lg:hidden mt-8 rounded-xl border border-white/[0.08] overflow-hidden">
                <button
                  type="button"
                ...
                >
                  ...
                  <ChevronDown className={`w-4 h-4 text-white/50 transition-transform duration-200 ${showMobilePreview ? "rotate-180" : ""}`} />
                </button>
                {showMobilePreview && (
                  <div className="p-5 pt-0 bg-dark border-t border-white/[0.08]">
                    <LivePreview form={form} />
                  </div>
                )}
              </div>

              <div className="mt-6 flex items-center justify-between pt-5 border-t border-white/[0.08]">
                <button onClick={prev}
                  className="font-headline text-[13px] font-bold uppercase tracking-widest text-white/50 hover:text-light flex items-center gap-2 transition-colors">
                ...
                <div className="flex items-center gap-3">
                  <button
                    ...
                    disabled={saving}
                    className="font-headline text-[13px] font-bold uppercase tracking-widest text-white/50 hover:text-light px-5 py-3 transition-colors disabled:opacity-40"
                  >
                    Save draft
                  </button>
            ...
            {/* Live preview */}
            <aside className="hidden lg:block border-l border-white/[0.08] bg-dark p-6 sticky top-[152px] h-[calc(100vh-152px)] overflow-y-auto">
              <LivePreview form={form} />
            </aside>
          </div>
        </div>
      ...
      {showCancelModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 overlay-in">
          ...
          <div className="relative bg-dark border border-white/[0.08] rounded-2xl shadow-2xl w-full max-w-sm p-7 modal-in">
            ...
            <div className="flex flex-col gap-3">
              <button
                ...
                disabled={saving}
                className="w-full font-headline text-[13px] font-bold uppercase tracking-widest px-6 py-3.5 rounded-md border border-lime-400 bg-lime-500/15 text-lime-400 hover:bg-lime-500/20 disabled:opacity-50 transition-colors flex items-center justify-center gap-2"
              >
                {saving ? "Saving…" : <><Check className="w-4 h-4" /> Save draft &amp; leave</>}
              </button>
              <button
                onClick={() => router.push("/organiser/dashboard")}
                className="w-full font-headline text-[13px] font-bold uppercase tracking-widest px-6 py-3.5 rounded-md border border-white/[0.08] text-white/50 hover:text-light hover:border-white/[0.08] transition-colors"
              >
                Discard &amp; leave
              </button>
              <button
                onClick={() => setShowCancelModal(false)}
                className="font-headline text-[12px] uppercase tracking-widest text-white/40 hover:text-white/60 transition-colors text-center py-1"
              >
                Keep editing
              </button>
            </div>

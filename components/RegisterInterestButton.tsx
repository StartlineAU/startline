...
export default function RegisterInterestButton({ eventId }: RegisterInterestButtonProps) {
  ...
  // Both states share the same primary-tinted outline shape; only the
  // background and label change. We start from the `outline` variant (so the
  // base styles don't include `bg-primary` from `default`) and override the
  // border colour, text colour, and padding to match the original design.
  const sharedClass =
